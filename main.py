...
def recommend():
    ...
    nearest_airport = request.form['nearestAirport']
    ...
    recommendations_response = client.chat.completions.create(
        model="gpt-3.5-turbo",
        messages=[
            {"role": "system", "content": "You will return only the names of the destinations separated by commas. Do not exceed 5 words. There should not be a period at the end of any destination. There should not be punctuation besides separated by commas."},
    ...
    return render_template('recommendations.html', nearest_airport=nearest_airport, recommendations=recommendations, images=images, input_themes=input_themes, recommended_themes=recommended_themes)
